...
panama_map <- get_stamenmap(
    bbox = c(left = -79.75, bottom = 8.75, right = -79.4, top =9.25), 
    maptype = "terrain",
    zoom = 10
)
```
...
ggmap(panama_map) +
  ...
  transition_reveal(time)+
  labs(title = "Heather's Panama City Triathalon",
       subtitle = "Time: {frame_along}")
 ...
 * Create a new variable that computes the number of new cases in the past week (HINT: use the `lag()` function you've used in a previous set of exercises). Replace missing values with 0's using `replace_na()`.  
  ...
  * Comment on what you observe.
  ...
  I notice that the lines that were originally on top on the static plot move change a lot faster on the animated plot. You can't really see the growth rate on the static plot, but animating it helps with that.
  

```r
covid_anim <- covid19 %>%
  group_by(state) %>% 
  mutate(lag7 = lag(cases, 7),
         lag7 = replace_na(lag7, 0),
         new_cases = cases- lag7) %>% 
  filter(cases >= 20) %>% 
  ...
  transition_reveal(date) +
  geom_point(aes(group = state, color = "blue")) +
...
I think the most interesting part of this animation is being able to see which states recorded cases first and how the rest of the states catch up to them.
...
```r
covidmapanim <- ggplot(data = covid_with_2018_pop_est) +
...
animate(covidmapanim, nframes = 200, end_pause = 10)

anim_save("CovidStates.gif")
```


```r
knitr::include_graphics("CovidStates.gif")
```
...
[Link To My Github](https://github.com/kkechely/KENNEDY_5_weekly_exercises/blob/main/05_exercises.md)
